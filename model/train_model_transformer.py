...
def Transformer_model(WINDOW_SIZE):
    ...
    input = Input(shape = (WINDOW_SIZE,INPUT_FEAT), name = 'input')
    input1 = input

    for _ in range(4):
        input1 = transformer_encoder(input1, 256, 4, 4, 0.25)

    x = layers.GlobalAveragePooling1D(data_format="channels_first")(input1)
    for dim in [128]:
    ...
    for r in range(rep):
        ...
        for k in range(Kfold):
            ...
            callbacks = [
                ...
                # Saving best model
                ModelCheckpoint('weights-best_transformer_k{}_r{}.hdf5'.format(k, r), monitor='val_loss', save_best_only=True,
                                verbose=1),
            ]
            # Load model
